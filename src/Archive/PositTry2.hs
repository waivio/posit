...
instance forall es.(KnownNat es) => Num (Posit es) where
  ...
  p1 + p2 = pLift2 (+) p1 p2
  -- p1 + p2 = pRound $ pLift2 (+) p1 p2  -- If we try to round, implementation details polute the type system.
...
encode :: forall es.(KnownNat es, Bits (IntN es), Bounded (IntN es), Num (IntN es)) => Posit es -> IntN es
...
buildIntRep :: (KnownNat es, Bits (IntN es), Num (IntN es)) => Posit es -> IntN es
buildIntRep p =
...
mkIntRep :: forall es.(KnownNat es, Bits (IntN es), Num (IntN es)) => Integer -> Natural -> Posit es -> IntN es
mkIntRep r e p =
...
formExponent :: forall es.(KnownNat es, Bits (IntN es), Num (IntN es)) => Posit es -> Natural -> Integer -> (IntN es, Integer)
...
decode :: forall es.(KnownNat es, Bits (IntN es), Integral (IntN es), Bounded (IntN es)) => IntN es -> Posit es
...
regime2Integer :: forall es.(KnownNat es, Bits (IntN es), Integral (IntN es)) => IntN es -> (Integer, Int)
...
findRegimeFormat :: forall es.(KnownNat es, Bits (IntN es)) => IntN es -> Bool
...
exponent2Nat :: forall es.(KnownNat es, Bits (IntN es), Integral (IntN es)) => Int -> IntN es -> Natural
...
type family IntN (es :: Nat) = int | int -> es where
  ...
  IntN 4 = Int128
  IntN 5 = Int256
  -- IntN x = mkDoubleWord ...? -- would be pretty cool if it could TH everything else


-- so that,
-- *Data.Posit> :instances IntN 0
...
instance forall es.(KnownNat es) => Num (IntN es) where
  fromInteger int = undefined
